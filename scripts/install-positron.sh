...
# Package sources. Point these at a git URL (optionally @branch) or a PyPI spec
# to control exactly what gets installed.
POSITRON_SERVER_PKG="${POSITRON_SERVER_PKG:-jupyter-positron-server}"
POSITRON_VERIFIER_PKG="${POSITRON_VERIFIER_PKG:-jupyter-positron-verifier}"
...
if [ -n "$POSITRON_CONFIG_SRC" ]; then
    ...
    umask 022
    cat > "$config_dest" <<EOF
# Generated by install-positron-tljh.sh — do not edit by hand; re-run the
