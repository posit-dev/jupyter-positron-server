...
|----------|-------------|
| `POSITRON_LICENSE_KEY_FILE` | Path to your Positron license key file. This environment variable requires the user who is starting Positron to have write access to wherever Positron Server located, since it will place the license in a location where Positron Server expects it. Positron Server will look for a license at `<positron-server-path>/resources/activation/linux/{ARCH}/license.lic` (where `{ARCH}` is `x86_64` or `aarch64`) if not set. |
